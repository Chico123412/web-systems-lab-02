import type { LibraryManager } from "../services/LibraryManager";
...
export function renderApp(
  ...
  subtitle.textContent = "Облік книг, користувачів, позичань і повернень";
  ...
  const bookForm = new BookForm((formData) => {
    try {
      ...
      NotificationService.success(`Книгу «${formData.title}» успішно додано`);
  ...
  const userForm = new UserForm((formData) => {
    try {
      libraryManager.addUser(formData.id, formData.name, formData.email);
  ...
  const userList = new UserList({
    onDelete: (userId: string): void => {
      try {
        ...
        NotificationService.success("Користувача успішно видалено");
