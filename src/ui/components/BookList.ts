...
export class BookList {
  ...
  render(books: Book[]): HTMLElement {
    ...
    const searchInput = section.querySelector<HTMLInputElement>("#book-search");
    const listContainer = section.querySelector<HTMLElement>(".book-list");
    ...
    const renderPagination = (
      ...
      for (let page = 1; page <= totalPages; page += 1) {
        ...
        pageButton.setAttribute("aria-label", `Перейти на сторінку ${page}`);
    ...
    const renderBooks = (): void => {
      ...
      if (booksForCurrentPage.length === 0) {
        ...
        emptyMessage.className = "text-secondary text-center py-4 mb-0";
      ...
      booksForCurrentPage.forEach((book) => {
        ...
        details.textContent = `${book.author}, ${book.publicationYear}`;
